...
class DVAE(nn.Module):
    ...
    def forward(self, x, phase):
        mu, logvar = self.encode(x) # --> latent variable: mu and logvar
        z = self.sample(mu, logvar, phase) # sampling from distribution (get any mean from this distibution)
        ref = self.decode(z) # decoding from mu which is samplied
